When you start training, expect three kinds of output: terminal/log messages, files in `output/...`, and eventually checkpoint model files.

Your config saves here:

```yaml
save_model_dir: ./output/PP-OCRv5_server_rec
```

So training output goes to:

```text
output/PP-OCRv5_server_rec/
```

At the beginning you should see logs like:

```text
train with paddle ... and device Place(cpu)
Initialize indexes of datasets: [...]
train dataloader has ... iters
valid dataloader has ... iters
train from scratch
```

During training you should see lines like:

```text
epoch: [1/1], global_step: 10, lr: ..., acc: ..., norm_edit_dis: ..., CTCLoss: ..., NRTRLoss: ..., loss: ...
```
...
| Output | Meaning |
|---|---|
| `epoch` | Which pass over the dataset you are on. |
| `global_step` | Batch number since training started. |
| `lr` | Current learning rate. |
| `acc` | Exact text-match accuracy. |
| `norm_edit_dis` | Text similarity score. Useful when predictions are close but not exact. |
| `CTCLoss` | Loss for the CTC recognition head. |
| `NRTRLoss` | Loss for the transformer recognition head. |
| `loss` | Combined total loss. |
| `ips` | Images per second. |
| `eta` | Estimated remaining time. |

At evaluation points, because your config has:

```yaml
eval_batch_step: [0, 500]
```

you should see validation every 500 steps:

```text
cur metric, acc: ..., norm_edit_dis: ..., fps: ...
best metric, acc: ..., norm_edit_dis: ...
```

Files you should expect:

```text
output/PP-OCRv5_server_rec/train.log
output/PP-OCRv5_server_rec/config.yml
```

After training reaches a save point, you should also get:

```text
output/PP-OCRv5_server_rec/latest.pdparams
output/PP-OCRv5_server_rec/latest.pdopt
output/PP-OCRv5_server_rec/latest.states
```

If validation runs and finds the best model so far:

```text
output/PP-OCRv5_server_rec/best_accuracy.pdparams 
output/PP-OCRv5_server_rec/best_accuracy.pdopt
output/PP-OCRv5_server_rec/best_accuracy.states
```
...
or, if no best model yet:

```text
latest.pdparams
```

That is the trained model weights.

One warning from your previous log: on CPU, your training was extremely slow and did not reach a save point. So you may see logs and metrics for a while but no `.pdparams` file until it finishes an epoch or reaches a checkpoint save condition.
