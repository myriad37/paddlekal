...
def eval(
    ...
    if config is not None:#
        ...
        rec_log_num = global_config.get("eval_sample_log_num", rec_log_num)#
        if model_type == "rec":
            save_model_dir = global_config.get("save_model_dir", "./output")
            eval_sample_dir = os.path.join(save_model_dir, "eval_samples")
            os.makedirs(eval_sample_dir, exist_ok=True)
            step_name = (
                "step_{}".format(global_step) if global_step is not None else "eval"
            )
            rec_save_file = os.path.join(eval_sample_dir, "{}.tsv".format(step_name))
            with open(rec_save_file, "w", encoding="utf-8") as fout:
                fout.write("ground_truth\tprediction\tconfidence\tcorrect\n")
    with paddle.no_grad():
        total_frame = 0.0
        total_time = 0.0
        pbar = tqdm(
        ...
        for idx, batch in enumerate(valid_dataloader):
            ...
            if model_type in ["table", "kie"]:
                ...
                if model_type == "rec" and rec_save_file is not None:
                    pred_list, label_list = post_result
                    with open(rec_save_file, "a", encoding="utf-8") as fout:
                        for pred_item, label_item in zip(pred_list, label_list):
                            pred_text = pred_item[0]
                            confidence = pred_item[1] if len(pred_item) > 1 else 0
                            gt_text = label_item[0]
                            correct = pred_text == gt_text
                            row = (gt_text, pred_text, confidence, correct)
                            if len(rec_sample_rows) < rec_log_num:
                                rec_sample_rows.append(row)
                            fout.write(
                                "{}\t{}\t{}\t{}\n".format(
                                    gt_text, pred_text, confidence, correct
                                )
                            )
                eval_class(post_result, batch_numpy)
        ...
        if rec_sample_rows:
            ...
            logger.info("-" * 100)
            for gt_text, pred_text, confidence, correct in rec_sample_rows:
                logger.info(
                    "{:<40} | {:<40} | {:<12.6f} | {}".format(
                        gt_text[:40], pred_text[:40], float(confidence), correct
                    )
                )
            logger.info("-" * 100)
            logger.info("saved eval prediction samples to {}".format(rec_save_file))
